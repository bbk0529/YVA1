...
def itemsParsing(items, so, po) :
    ...
    for row in range(start,len(items)) :
        ...
        for i in range(2,n) :
            ...
            TotalList.append([so, soidx, po, poidx, (so+ str(no)).strip(), d1,t1,d2,t2,d3,t3,d4,t4,reference])
        ...
        SumList.append([so, soidx, po, poidx, (so+ str(no)).strip(), partno, config, T1,T2,T3,T4, delta, stored, etd, eta, status,REMARK + " " + reference, reference, partial])
